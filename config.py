...
# Постобработка: авто-баланс белого после раскрашивания ("серый мир").
# Убирает общий цветовой сдвиг — например, зеленоватый/холодный налёт на коже.
ENABLE_AUTO_WHITE_BALANCE = True
# Сила коррекции: 0.0 = выключено, 1.0 = полная. 0.6-0.8 обычно безопасно.
# Снижайте на фото с одним доминирующим реальным цветом (закат, листва, море),
# иначе баланс может "вымыть" этот цвет, посчитав его за паразитный сдвиг.
AUTO_WHITE_BALANCE_STRENGTH = 0.7
